...
/**
 * "Experiment holds all the information about the fpga sequence as it is being
 * built, and knows how to produce the memory and sram instructions that
 * actually get sent out to run the sequence."
 * 
 * For the ADC addition, we now have to be careful to only perform things like
 * memory ops on DAC FpgaModels, not ADC ones.
 * 
 * @author maffoo
 * @author pomalley
 */
public class Experiment {
	...
	public Device getDevice(String name) {
		Preconditions.checkArgument(devicesByName.containsKey(name), "Device '%s' not found.", name);
		...
		public boolean[] interpretData(Data data) {
			if (isAdc()) {
				Preconditions.checkArgument(data.matchesType("(*i, *i)"),
						"interpretData called with data type %s on an ADC channel. Qubit Sequencer mixup.",
				...
				Preconditions.checkArgument(data.matchesType("*w"),
						"interpretData called with data type %s on a DAC channel. Qubit Sequencer mixup.",
						data.getType().toString());
	...
	/**
	 * Add a delay in the memory sequence. Only applies to DACs.
	 */
	...
	/**
	 * Start timer on a set of boards. This only applies to DAC fpgas.
	 */
	...
	/**
	 * Get the proper length for an SRAM block after padding. The length should be a
	 * multiple of 4 and greater or equal to 20.
	 * 
	 * @param name
	 * @return
	 */
