...
public abstract class FpgaModelDac implements FpgaModel {
	...
	/**
	 * Check whether the timer is currently running (has been started but not yet
	 * stopped)
	 * 
	 * @return
	 */
	...
	/**
	 * Check that the timer status of this board is ok, namely that the timer has
	 * been started at least once and stopped as many times as it has been started.
	 * This ensures that all boards will be run properly.
	 */
	...
	/**
	 * Issue a start timer command. Will only succeed if the timer is currently
	 * stopped.
	 */
	...
	/**
	 * Issue a stop timer command. Will only succeed if the timer is currently
	 * running.
	 */
	...
	/**
	 * Get the bits for the full SRAM sequence for this board. We loop over all
	 * called blocks, padding the bits from each block and then concatenating them
	 * together.
	 * 
	 * @return
	 */
	...
	/**
	 * Get DAC sram bits for a particular block. This is implemented differently for
	 * different board configurations (analog vs. microwave) so it is deferred to
	 * concrete subclasses.
	 * 
	 * @param block
	 * @return
	...
	private void setTriggerBits(long[] s, String block, boolean addAutoTrigger) {
		...
		// TODO define dummy trigger channels, just like we do with Microwave and analog
		// channels
	...
	/**
	 * Pad an array to the given length by repeating the first value the specified
	 * number of times
	 * 
	 * @param data
	 * @param len
