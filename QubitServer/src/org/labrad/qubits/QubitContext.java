...
public class QubitContext extends AbstractServerContext {
	...
	/**
	 * Get a channel from the experiment that is of a particular Channel class this
	 * unpacks the channel descriptor directly from the incoming LabRAD data
	 */
	...
	/**
	 * Get a channel from the experiment that is of a particular class. In this
	 * case, no channel name is specified, so this will succeed only if there is a
	 * unique channel of the appropriate type.
	 */
	...
	/**
	 * Build a setup packet from a given set of records, using the predefined setup
	 * context
	 * 
	 * @param data
	 */
	private Data buildSetupPacket(String server, Data records) {
		return Data.clusterOf(Data.clusterOf(Data.valueOf(setupContext.getHigh()), Data.valueOf(setupContext.getLow())),
				Data.valueOf(server), records);
	}
	...
	private void checkSetupPacket(Data packet) {
		if (!packet.matchesType("(ww)s?")) {
			Failure.fail("Setup packet has invalid format: " + "Expected ((ww) s ?{records}) but got %s.",
					packet.getTag());
		}
		Data records = packet.get(2);
		if (!records.isCluster()) {
			Failure.fail("Setup packet has invalid format: " + "Expected a cluster of records but got %s.",
					records.getTag());
		}
		for (int i = 0; i < records.getClusterSize(); i++) {
			...
			if (!record.matchesType("s?")) {
				Failure.fail(
						"Setup packet has invalid format: "
	...
	// Echo
	//
	@Setting(id = 99, name = "Echo", doc = "Echo back.")
	@Returns("?")
	...
	@Setting(id = 100, name = "Initialize", doc = "Initialize a new sequence with the given device and channel setup."
	...
	@Setting(id = 200, name = "New Config", doc = "Clear all config calls in this context." + "\n\n"
	...
	@Setting(id = 210, name = "Config Microwaves", doc = "Configure the Anritsu settings for the given channel."
	...
	public void config_microwaves(@Accepts({ "s", "ss" }) Data id, @Accepts("v[GHz]") double freq,
	...
	@Setting(id = 220, name = "Config Preamp", doc = "Configure the preamp settings for the given channel." + "\n\n"
			+ "This includes the DAC offset and polarity, as well as high- " + "and low-pass filtering.")
	public void config_preamp(@Accepts({ "s", "ss" }) Data id, long offset, boolean polarity, String highPass,
			String lowPass) {
	...
	@Setting(id = 230, name = "Config Settling", doc = "Configure the deconvolution settling rates for the given channel.")
	public void config_settling(@Accepts({ "s", "ss" }) Data id, @Accepts("*v[GHz]") double[] rates,
	...
	@Setting(id = 240, name = "Config Timing Order", doc = "Configure the order in which timing results should be returned."
			+ "\n\n" + "If this option is not specified, timing results will be returned "
			+ "for all timing channels in the order they were defined when this " + "sequence was initialized.")
	...
	@Setting(id = 250, name = "Config Setup Packets", doc = "Configure other setup state and packets for this experiment."
	...
	@Setting(id = 260, name = "Config Autotrigger", doc = "Configure automatic inclusion of a trigger pulse on every SRAM sequence."
			...
			+ "be rounded to the nearest ns.  If no length is specified, the default (16 ns) " + "will be used.")
	...
	@Setting(id = 280, name = "Config Switch Intervals", doc = "Configure switching intervals for processing timing data from a particular timing channel.")
	...
	@Setting(id = 290, name = "Config Critical Phases", doc = "Configure the critical phases for processing ADC readout. Takes 'v' for average mode, '*v' for demod mode "
	...
	@Setting(id = 300, name = "New Mem", doc = "Clear memory content in this context.")
	public void new_mem() {
	 ...
	 * @param microseconds
	 */
	@Setting(id = 310, name = "Mem Bias", doc = "Adds Bias box commands to the specified channels." + "\n\n"
	...
	@Setting(id = 320, name = "Mem Delay", doc = "Add a delay to all channels.")
	public void mem_delay(@Accepts("v[us]") double delay) {
		getExperiment().addMemoryDelay(delay);
		memDirty = true;
	}

	@Setting(id = 330, name = "Mem Call SRAM", doc = "Call the SRAM block specified by name." + "\n\n"
	...
	@Setting(id = 340, name = "Mem Start Timer", doc = "Start the timer for the specified timing channels.")
	...
	@Setting(id = 350, name = "Mem Stop Timer", doc = "Stop the timer for the specified timing channels.")
	 ...
	 */
	@Setting(id = 400, name = "New SRAM Block", doc = "Create a new SRAM block with the specified name and length."
			...
			+ "a multiple of 4, the data will be padded at the beginning after " + "deconvolution.")
	...
	@Setting(id = 401, name = "SRAM Dual Block Delay", doc = "Set the delay between the first and second blocks of a dual-block SRAM command."
			...
			+ "nanoseconds which may give unexpected results if the delay is converted from " + "another unit of time.")
	 ...
	 */
	@Setting(id = 410, name = "SRAM IQ Data", doc = "Set IQ data for the specified channel." + "\n\n"
			+ "The data is specified as a list of complex numbers, "
			+ "with the real and imaginary parts giving the I and Q "
			+ "microwave quadratures.  The length of the data should " + "match the length of the current SRAM block.  "
			+ "An optional boolean specifies whether the data " + "should be deconvolved (default: true).")
	public void sram_iq_data(@Accepts({ "s", "ss" }) Data id, @Accepts("*c") Data vals) {
		sram_iq_data(id, vals, true);
		sramDirty = true;
	}

	@SettingOverload
	public void sram_iq_data(@Accepts({ "s", "ss" }) Data id, @Accepts("*c") Data vals, boolean deconvolve) {
		IqChannel ch = getChannel(id, IqChannel.class);
	 ...
	 */
	@Setting(id = 411, name = "SRAM IQ Data Fourier", doc = "Set IQ data in Fourier representation for the specified channel."
			+ "\n\n" + "The data is specified as a list of complex numbers, "
			+ "with the real and imaginary parts giving the I and Q "
			+ "microwave quadratures.  The length of the data should " + "match the length of the current SRAM block.")
	public void sram_iq_data_fourier(@Accepts({ "s", "ss" }) Data id, @Accepts("*c") Data vals,
	 ...
	 */
	@Setting(id = 420, name = "SRAM Analog Data", doc = "Set analog data for the specified channel." + "\n\n"
	...
	public void sram_analog_data(@Accepts({ "s", "ss" }) Data id, @Accepts("*v") Data vals) {
		sram_analog_data(id, vals, true);
		sramDirty = true;
	}

	@SettingOverload
	public void sram_analog_data(@Accepts({ "s", "ss" }) Data id, @Accepts("*v") Data vals, boolean deconvolve) {
		AnalogChannel ch = getChannel(id, AnalogChannel.class);
	 ...
	 */
	@Setting(id = 421, name = "SRAM Analog Data Fourier", doc = "Set analog data in Fourier representation for the specified channel."
	...
	public void sram_analog_fourier_data(@Accepts({ "s", "ss" }) Data id, @Accepts("*c") Data vals,
			@Accepts("v[ns]") double t0) {
		AnalogChannel ch = getChannel(id, AnalogChannel.class);
		ComplexArray c = ComplexArray.fromData(vals);
	...
	@Setting(id = 430, name = "SRAM Trigger Data", doc = "Set trigger data for the specified trigger channel")
	public void sram_trigger_data(@Accepts({ "s", "ss" }) Data id, @Accepts("*b") Data data) {
		TriggerChannel ch = getChannel(id, TriggerChannel.class);
		ch.addData(currentBlock, new TriggerDataTime(data.getBoolArray()));
		sramDirty = true;
	}

	@Setting(id = 431, name = "SRAM Trigger Pulses", doc = "Set trigger data as a series of pulses for the specified trigger channel"
			+ "\n\n" + "Each pulse is given as a cluster of (start, length) values, " + "specified in nanoseconds.")
	public void sram_trigger_pulses(@Accepts({ "s", "ss" }) Data id, @Accepts("*(v[ns] v[ns])") List<Data> pulses) {
		TriggerChannel ch = getChannel(id, TriggerChannel.class);
	...
	@Setting(id = 510, name = "Set Start Delay", doc = "Sets the hdgdgfdghdgfdghsdfstart delay for this channel; must be an ADC or IQ channel. "
			+ "First argument {s or (ss)}: channel (either device name or (device name, channel name) "
			+ "Second: delay, in clock cycles (w)")
	public void adc_set_start_delay(@Accepts({ "s", "ss" }) Data id, @Accepts({ "w", "v[us]" }) Data delay) {
	...
	@Setting(id = 520, name = "ADC Set Mode", doc = "asdfasdfasfasdfSets the mode of this channel.\n "
			+ "First argument {s or (ss)}: channel (either device name or (device name, channel name)\n "
			+ "Second {s}: either 'demodulate' or 'average'")
	public void adc_set_mode(@Accepts({ "s", "ss" }) Data id, @Accepts("s") Data mode) {
		AdcChannel ch = getChannel(id, AdcChannel.class);
		ch.setMode(mode.getString());
	}

	@Setting(id = 530, name = "ADC Set Filter Function", doc = "Sets the filter function for this channel; must be an adc channel in demod mode.\n "
			+ "First argument {s or (ss)}: channel (either device name or (device name, channel name)\n "
	...
	public void adc_set_filter_function(@Accepts({ "s", "ss" }) Data id, @Accepts("s") Data bytes,
			@Accepts("w") Data stretchLen, @Accepts("w") Data stretchAt) {
		AdcChannel ch = getChannel(id, AdcChannel.class);
		ch.setFilterFunction(bytes.getString(), (int) stretchLen.getWord(), (int) stretchAt.getWord());
	}

	@Setting(id = 531, name = "ADC Set Trig Magnitude", doc = "Sets the filter function for this channel. Must be an adc channel in demod mode "
	...
	public void adc_set_trig_magnitude(@Accepts({ "s", "ss" }) Data id, @Accepts("w") Data channel,
			@Accepts("w") Data sineAmp, @Accepts("w") Data cosineAmp) {
		AdcChannel ch = getChannel(id, AdcChannel.class);
		ch.setTrigMagnitude((int) channel.getWord(), (int) sineAmp.getWord(), (int) cosineAmp.getWord());
	}

	@Setting(id = 532, name = "ADC Demod Phase", doc = "Sets the demodulation phase. Must be an adc channel in demod mode "
	...
	public void adc_demod_phase(@Accepts({ "s", "ss" }) Data id, @Accepts("w") Data channel,
	...
	@Setting(id = 900, name = "Build Sequence", doc = "Compiles SRAM and memory sequences into runnable form." + "\n\n"
	...
	public void build_sequence() throws InterruptedException, ExecutionException {
		...
		// TODO if a microwave source is used for dummy channels and real channels,
		// resolve here
		...
		// this is the new-style deconvolution routine which sends all deconvolution
		// requests in separate packets
		...
		for (FpgaModelAdc fpga : expt.getAdcFpgas()) {
			...
			// Preconditions.checkState(conf != null, "ADC channel '%s' was not
			// configured!", fpga.getChannel().getName());
		...
		for (FpgaModelDac fpga : expt.getDacFpgas()) {
			...
			if (fpga.hasDualBlockSram()) {
				runRequest.add("SRAM dual block", Data.valueOf(fpga.getSramDualBlock1()),
						Data.valueOf(fpga.getSramDualBlock2()), Data.valueOf(fpga.getSramDualBlockDelay()));
			} else {
		...
		// run the sequence
		runIndex = runRequest.addRecord("Run Sequence", Data.valueOf(0L), // put in a dummy value for number of reps
				Data.valueOf(true), // return timing results
				Data.clusterOf(setupPackets), Data.listOf(setupState, Setters.stringSetter));
	...
	@Setting(id = 1009, name = "Get SRAM Final", doc = "Make sure to run build sequence first.")
	...
	@Setting(id = 1000, name = "Run", doc = "Runs the current sequence by sending to the GHz DACs server." + "\n\n"
	...
	@Setting(id = 1001, name = "Run DAC SRAM", doc = "Similar to 'Run', but does dac_run_sram instead of run_sequence. This simply runs the "
			+ "DAC SRAM (once or repeatedly) and gives no data back. Used to determine the power output, etc.")
	public void run_dac_sram(@Accepts("*w") Data data, @Accepts("b") Data loop)
	...
	@Setting(id = 1100, name = "Get Data Raw", doc = "Gets the raw timing data from the previous run. Given a deinterlace argument, only DAC data are returned")
	@Returns({ "*2w", "?" })
	...
	@Setting(id = 1101, name = "Get Data Raw Microseconds", doc = "Gets the raw timing data from the previous run, converted to microseconds")
	@Returns("*2v[us]")
	...
	@Setting(id = 1102, name = "Get Data Raw Switches", doc = "Gets the raw timing data from the previous run, converted to "
	...
	@Setting(id = 1110, name = "Get Data Probs Separate", doc = "Get independent switching probabilities from the previous run."
	...
	@Setting(id = 1111, name = "Get Data Probs", doc = "Get combined switching probabilities from the previous run."
	...
	@Setting(id = 1112, name = "Get Data Raw Phases", doc = "Gets the raw data from the ADC results, converted to phases.")
	@Returns("*2v[rad]")
	...
	@Setting(id = 2001, name = "Dump Sequence Packet", doc = "Returns a dump of the packet to be sent to the GHz DACs server."
	...
	public Data dump_packet() {
		...
		for (Record r : nextRequest.getRecords()) {
			records.add(Data.clusterOf(Data.valueOf(r.getName()), r.getData()));
		}
		return Data.clusterOf(records);
	}

	@Setting(id = 2002, name = "Dump Sequence Text", doc = "Returns a dump of the current sequence in human-readable form")
	@Returns("s")
