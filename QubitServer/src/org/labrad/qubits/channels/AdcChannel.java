...
public class AdcChannel implements Channel, TimingChannel, StartDelayChannel {
	...
	public void setFpgaModel(FpgaModel fpga) {
		Preconditions.checkArgument(fpga instanceof FpgaModelAdc, "AdcChannel '%s' requires ADC board.", getName());
	...
	public void setMode(String mode) {
		...
		// we use Locale.ENGLISH because goddamit I want this code to work if we go to
		// russia or some shit.
	}
	...
	// these are passthroughs to the config object. in most cases we do have to
	// check that
	...
	public void setFilterFunction(String filterFunction, int stretchLen, int stretchAt) {
		Preconditions.checkState(mode == AdcMode.DEMODULATE,
				"Channel must be in demodulate mode for setFilterFunction to be valid.");
	...
	public void setTrigMagnitude(int channel, int ampSin, int ampCos) {
		Preconditions.checkState(mode == AdcMode.DEMODULATE,
				"Channel must be in demodulate mode for setTrigMagnitude to be valid.");
		((AdcDemodConfig) config).setTrigMagnitude(channel, ampSin, ampCos);
	}

	public void setPhase(int channel, int dPhi, int phi0) {
		Preconditions.checkState(mode == AdcMode.DEMODULATE,
				"Channel must be in demodulate mode for setPhase to be valid.");
	...
	public void turnChannelOn(int channel) {
		Preconditions.checkState(mode == AdcMode.DEMODULATE,
				"Channel must be in demodulate mode for turnChannelOn to be valid.");
		((AdcDemodConfig) config).turnChannelOn(channel);
	}

	public void turnChannelOff(int channel) {
		Preconditions.checkState(mode == AdcMode.DEMODULATE,
				"Channel must be in demodulate mode for turnChannelOff to be valid.");
		((AdcDemodConfig) config).turnChannelOff(channel);
	}

	public boolean[] getChannelUsage(int channel) {
		Preconditions.checkState(mode == AdcMode.DEMODULATE,
				"Channel must be in demodulate mode for getChannelUsage to be valid.");
