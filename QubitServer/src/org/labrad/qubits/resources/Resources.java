...
/**
 * "Resources represent the available hardware; these are configured in the
 * registry."
 * 
 * @author maffoo
 */
public class Resources {
	...
	public <T extends Resource> T get(String name, Class<? extends T> cls) {
		Preconditions.checkArgument(resources.containsKey(name), "Resource '%s' not found", name);
		Resource r = resources.get(name);
		Preconditions.checkArgument(cls.isInstance(r), "Resource '%s' not of type %s", name, cls.getName());
		return (T) r;
	}
	...
	public static Resource create(DeviceType type, String name, List<Data> properties) {
		switch (type) {
		case UWAVEBOARD:
			return MicrowaveBoard.create(name);
		case ANALOGBOARD:
			return AnalogBoard.create(name);
		case FASTBIAS:
			return FastBias.create(name, properties);
		case PREAMP:
			return PreampBoard.create(name);
		case UWAVESRC:
			return MicrowaveSource.create(name);
		case ADCBOARD:
			return AdcBoard.create(name);
		default:
	...
	public static void updateWiring(List<Data> resources, List<Data> fibers, List<Data> microwaves) {
		/*
		 * resources - [(String type, String id),...] fibers - [((dacName,
		 * fiber),(cardName, channel)),...]
		 */
