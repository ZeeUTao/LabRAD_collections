 ...
 * 
 * It is held by an AdcChannel object. The global parameters are start delay and
 * filter function. The per-channel parameters are the demod phase ((freq or
 * steps/cycle) and (phase or offset steps)) and the trigger magnitude (sine amp
 * and cosine amp, each a byte).
 * 
 * Note that this is a little different from the way that config objects are
 * normally done in this server. Normally the configs are for other servers
 * (e.g. microwave source) and are sent out as part of the GHz FPGA server's
 * "setup packets". Here the config is for a device on the GHz FPGA server and
 * is sent as part of the main run request.
 * 
 * @author pomalley
 *
 */
public class AdcDemodConfig extends AdcBaseConfig {
	...
	/**
	 * Each byte is the weight for a 4 ns interval. A single value can be repeated
	 * for a stretch in the middle. How long to repeat for is specified by
	 * stretchLen, and where to start repeating by stretchAt.
	 */
	String filterFunction;
	int stretchLen, stretchAt;

	/**
	 * Each channel has a dPhi, the number of addresses to step through per time
	 * sample.
	 */
	int dPhi[];
	...
	/**
	 * ampSin and ampCos are the magnitude of the cos and sin functions of a given
	 * channel
	 */
	int ampSin[], ampCos[];
	...
	public AdcDemodConfig(String channelName, Map<String, Long> buildProperties) {
		...
		// see fpga server documentation on the "ADC Demod Phase" setting for an
		// explanation of the two below.
	...
	/*
	 * private int numChannelsInUse() { int n = 0; for (boolean b : inUse) if (b)
	 * n++; return n; }
	 */
	...
	public void setCriticalPhases(double[] criticalPhases) {
		Preconditions.checkArgument(criticalPhases.length == MAX_CHANNELS,
				"Number of critical phases must = number of channels");
	...
	public void setCriticalPhase(int channelIndex, double criticalPhase) {
		Preconditions.checkArgument(channelIndex >= 0 && channelIndex < MAX_CHANNELS,
				"channelIndex must be >= 0 and < MAX_CHANNELS");
		Preconditions.checkArgument(criticalPhase >= -Math.PI && criticalPhase <= Math.PI,
	...
	public boolean[] interpretPhases(int[] Is, int[] Qs, int channel) {
		...
		Preconditions.checkArgument(inUse[channel], "Interpret phases on channel %s -- channel not turned on!",
				channel);
		// System.out.println("interpretPhases: channel " + channel + " crit phase: " +
		// criticalPhase[channel]);
	...
	public void setPhase(int channel, int dPhi, int phi0) {
		Preconditions.checkArgument(channel <= MAX_CHANNELS, "channel must be <= %s", MAX_CHANNELS);
		// Preconditions.checkArgument(phi0 >= 0 && phi0 < (int)Math.pow(2,
		// LOOKUP_ACCUMULATOR_BITS),
	...
	public void setPhase(int channel, double frequency, double phase) {
		...
		int dPhi = (int) Math
				.floor(frequency * Math.pow(2, LOOKUP_ACCUMULATOR_BITS) * DEMOD_TIME_STEP * Math.pow(10, -9.0));
		int phi0 = (int) (phase * Math.pow(2, LOOKUP_ACCUMULATOR_BITS) / (2 * Math.PI));
		setPhase(channel, dPhi, phi0);
	}

	/**
	 * In the demod case, the following packets are added: ADC Run Mode Start Delay
	 * ADC Filter Func for each channel: ADC Demod Phase ADC Trig Magnitude
	 * 
	...
	public void addPackets(Request runRequest) {
		...
		Preconditions.checkState(stretchLen > -1 && stretchAt > -1, "ADC Filter Func not set for channel '%s'",
				this.channelName);
		boolean oneFound = false;
		for (int i = 0; i < MAX_CHANNELS; i++) {
			if (inUse[i]) {
				oneFound = true;
				// Preconditions.checkState(phi0[i] > -1, " %s on channel '%s'", i,
				// this.channelName);
				Preconditions.checkState(ampSin[i] > -1 && ampCos[i] > -1,
						"ADC Trig Magnitude not set on activated demod channel %s on channel '%s'", i,
						this.channelName);
			}
		}
		...
		runRequest.add("ADC Filter Func", Data.valueOf(this.filterFunction), Data.valueOf((long) this.stretchLen),
				Data.valueOf((long) this.stretchAt));
		for (int i = 0; i < MAX_CHANNELS; i++) {
			if (inUse[i]) {
				runRequest.add("ADC Demod Phase", Data.valueOf((long) i), Data.valueOf(dPhi[i]), Data.valueOf(phi0[i]));
				runRequest.add("ADC Trig Magnitude", Data.valueOf((long) i), Data.valueOf((long) ampSin[i]),
						Data.valueOf((long) ampCos[i]));
			}
		}
	}
}
