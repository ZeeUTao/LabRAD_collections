...
public class PreampConfig {
	private static final String[] highPassFilterNames = new String[] { "DC", "3300", "1000", "330", "100", "33", "10",
			"3.3" };
	private static final long[] highPassFilterVals = new long[] { 0, 1, 2, 3, 4, 5, 6, 7 };
	private static final Map<String, Long> highPassFilterMap = Maps.newHashMap();
	private static final String highPassAllowedNames;

	private static final String[] lowPassFilterNames = new String[] { "0", "0.22", "0.5", "1", "2.2", "5", "10", "22" };
	private static final long[] lowPassFilterVals = new long[] { 0, 1, 2, 3, 4, 5, 6, 7 };
	...
	public SetupPacket getSetupPacket(PreampChannel ch) {
		...
		data.get(2).setString("Register", 0).setString(ch.getPreampChannel().toString().toUpperCase(), 1, 0)
				.setWord(highPass, 1, 1, 0).setWord(lowPass, 1, 1, 1).setWord(polarity ? 1L : 0L, 1, 1, 2)
