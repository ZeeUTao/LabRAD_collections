...
/**
 * Model of a device, essentially a collection of named channels of various
 * types. Each channel connects various physical devices (FastBias, Preamp,
 * GHzDac) to some controllable parameter on the device (flux bias, microwave,
 * etc.).
 * 
 * @author maffoo
 */
public class Device {
	...
	public Channel getChannel(String name) {
		Preconditions.checkArgument(channelsByName.containsKey(name), "Device '%s' has no channel named '%s'",
				getName(), name);
	...
	public <T extends Channel> T getChannel(String name, Class<T> cls) {
		...
		Preconditions.checkState(cls.isInstance(ch), "Channel '%s' is not of type '%s'", name, cls.getName());
		return (T) ch;
	}

	/**
	 * Get a channel by type only. This fails if there is more than one channel with
	 * the desired type.
	 * 
	 * @param     <T>
	 * @param cls
	...
	public <T extends Channel> T getChannel(Class<T> cls) {
		...
		Preconditions.checkState(channels.size() == 1, "Device '%s' has more than one channel of type '%s'", getName(),
				cls.getName());
